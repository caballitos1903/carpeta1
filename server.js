...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // ⚠️ Mercado Pago puede enviar notificaciones con query params o body vacío
    const { type, data } = req.body || {};

    if (type === "payment" && data?.id) {
      const paymentId = data.id;

      // 🔎 Consultar pago real a la API de MP
...
app.listen(PORT, () => {
  console.log(
    `🚀 Servidor corriendo en ${isProduction ? "https://libertrades.xyz" : `http://localhost:${PORT}`}`
  );
});
